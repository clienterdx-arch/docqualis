    ...
    <div className="p-8 max-w-[1400px] mx-auto animate-in fade-in duration-500 relative print:p-0">
      ...
      <div className="flex items-center justify-between mb-8 print:hidden">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setIsExportModalOpen(true)} className="flex items-center gap-2 bg-white border border-slate-100 text-slate-700 px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-slate-50 shadow-sm transition-all"><FileSpreadsheet className="w-4 h-4 text-emerald-600" /> Exportar Lista Mestra</button>
      ...
      {viewState === "lista" && (
        <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
          <div className="flex items-center gap-4 mb-2">
            <button onClick={() => setViewState("blocos")} className="p-2 bg-white border border-slate-100 rounded-lg text-slate-500 hover:bg-slate-50 shadow-sm transition-all"><ArrowLeft className="w-5 h-5" /></button>
                      ...
                      {pastaAtiva === "Repositório" ? (
                        <div className="flex items-center justify-end gap-2">
                           <button 
                            ...
                            className="p-2 bg-white border border-slate-100 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm"
                           ...
                           <button 
                            ...
                            className="p-2 bg-white border border-slate-100 text-slate-600 rounded-lg hover:bg-slate-100 transition-all shadow-sm"
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {(pastaAtiva === "Em Elaboração" || pastaAtiva === "Rejeitados") && (
                            <Link href={`/editar-documento/${doc.id}`} onClick={(e) => e.stopPropagation()} className="px-3 py-1.5 bg-white border border-slate-100 text-slate-700 font-bold rounded-lg text-xs hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 shadow-sm transition-all inline-flex items-center justify-center gap-1.5">
                              <Edit className="w-3.5 h-3.5"/> Retomar
                            </Link>
                          )}
                          {(pastaAtiva === "Em Verificação" || pastaAtiva === "Em Homologação") && (
                            <Link href={`/documento/${doc.id}`} onClick={(e) => e.stopPropagation()} className="px-3 py-1.5 bg-white border border-slate-100 text-slate-700 font-bold rounded-lg text-xs hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 shadow-sm transition-all inline-flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4">
      <div className="hidden print:block mb-8">
        <h2 className="text-2xl font-black text-slate-900 border-b border-slate-100 pb-2">Relatório de Inspeção de Documentos</h2>
        <p className="text-sm text-slate-500 mt-2">Gerado em: {new Date().toLocaleString('pt-BR')}</p>
      </div>

      <div className="flex justify-end mb-6 print:hidden">
        <button onClick={gerarRelatorioPDF} className="flex items-center gap-2 bg-white border border-slate-100 text-slate-700 px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-slate-50 shadow-sm transition-all">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden print:border-none print:shadow-none">
...
function PainelConfiguracaoAdmin({ setMensagemSistema }: any) {
  ...
  const handleAdd = async () => {
    ...
    if (!dbError) { setNovoNome(""); setNovaSigla(""); setNovoDirId(""); carregarTudo(); }
  };
  ...
  if (isLoading) return <div className="text-center p-12 text-slate-500 font-bold flex justify-center bg-white rounded-xl border border-slate-100"><RefreshCw className="animate-spin w-6 h-6 mr-2"/> Carregando Módulo de Configurações...</div>;
