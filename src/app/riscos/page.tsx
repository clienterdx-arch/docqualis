...
export default function GestaoRiscosPage() {
  ...
  const carregarRiscos = useCallback(async () => {
    ...
    if (error) {
      setMensagem("Erro ao carregar riscos.");
      setIsLoading(false);
      return;
    }
  ...
  const riscosFiltrados = useMemo(() => {
    return riscos.filter((risco) => {
      const texto = `${risco.codigo} ${risco.titulo} ${risco.setor} ${risco.processo_vinculado} ${risco.responsavel}`.toLowerCase();
  ...
  const indicadores = useMemo(() => {
    ...
    const atrasados = riscos.filter((r) => r.prazo_tratamento && new Date(r.prazo_tratamento) < new Date()).length;
  ...
  function abrirNovo() {
    ...
    setRiscoEditando({ ...riscoInicial, codigo: `RIS-${proximo}` });
    setModalAberto(true);
  }
  ...
  function atualizarCampo(campo: keyof Risco, valor: any) {
  ...
  async function salvarRisco() {
    ...
    if (!riscoEditando.titulo || !riscoEditando.setor || !riscoEditando.responsavel) {
      setMensagem("Preencha título, setor e responsável.");
      return;
    }
    ...
    const payload = {
      ...
      classificacao: classificar(calcularScore(Number(riscoEditando.probabilidade), Number(riscoEditando.impacto))),
      ...
      risco_residual_probabilidade: Number(riscoEditando.risco_residual_probabilidade || 1),
      ...
      data_avaliacao: riscoEditando.data_avaliacao || new Date().toISOString().slice(0, 10),
      data_revisao: riscoEditando.data_revisao || null,
    };

    const { error } = riscoEditando.id
      ? await supabase.from("riscos").update(payload).eq("id", riscoEditando.id).eq("empresa_id", empresaId)
  ...
  async function excluirRisco(id?: string) {
    ...
    if (error) {
      setMensagem("Erro ao excluir risco.");
      return;
    }
  ...
  function exportarCsv() {
    ...
    a.href = url;
    a.download = `matriz-riscos-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const scoreAtual = calcularScore(Number(riscoEditando.probabilidade), Number(riscoEditando.impacto));
    ...
    <div className="p-8 max-w-[1600px] mx-auto bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-5 mb-8">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Gestão de Riscos</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        {[
          { label: "Total de riscos", value: indicadores.total, icon: <Layers className="w-5 h-5" />, color: "text-blue-700 bg-blue-50" },
          { label: "Críticos", value: indicadores.criticos, icon: <AlertTriangle className="w-5 h-5" />, color: "text-red-700 bg-red-50" },
          { label: "Altos", value: indicadores.altos, icon: <Gauge className="w-5 h-5" />, color: "text-orange-700 bg-orange-50" },
          { label: "Em tratamento", value: indicadores.tratamento, icon: <ClipboardCheck className="w-5 h-5" />, color: "text-violet-700 bg-violet-50" },
          { label: "Atrasados", value: indicadores.atrasados, icon: <CalendarClock className="w-5 h-5" />, color: "text-amber-700 bg-amber-50" },
        ].map((card) => (
          <div key={card.label} className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-6 mb-8">
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-black text-slate-900">Matriz de Mapeamento de Riscos</h2>
              <p className="text-xs text-slate-500 font-medium">Visão tabular semelhante à planilha de gestão institucional.</p>
            </div>
              ...
              <thead className="sticky top-0 z-10">
                <tr className="bg-slate-900 text-white text-[10px] uppercase tracking-widest">
                  {[
                    ...
                    <th key={h} className="px-4 py-3 text-left font-black border-r border-slate-700 last:border-r-0">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {riscosFiltrados.map((risco) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3 font-black text-blue-700">{risco.codigo}</td>
                      <td className="px-4 py-3 text-slate-700">{risco.processo_vinculado || "-"}</td>
                      <td className="px-4 py-3 text-slate-700">{risco.setor}</td>
                      <td className="px-4 py-3 font-bold text-slate-900">{risco.titulo}</td>
                      <td className="px-4 py-3 text-slate-600 max-w-[220px] truncate">{risco.causa || "-"}</td>
                      <td className="px-4 py-3 text-slate-600 max-w-[220px] truncate">{risco.consequencia || "-"}</td>
                      <td className="px-4 py-3 font-black text-center">{risco.probabilidade}</td>
                      <td className="px-4 py-3 font-black text-center">{risco.impacto}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-lg border text-xs font-black ${classeCelula(score)}`}>
                          {score}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-lg border text-xs font-black ${classeRisco(risco.classificacao)}`}>
                          {risco.classificacao}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-600 max-w-[220px] truncate">{risco.controles_existentes || "-"}</td>
                      <td className="px-4 py-3 text-slate-600 max-w-[260px] truncate">
                        {[risco.barreiras_preventivas, risco.barreiras_detectivas, risco.barreiras_corretivas].filter(Boolean).join(" | ") || "-"}
                      </td>
                      <td className="px-4 py-3 font-bold text-slate-700">{risco.estrategia}</td>
                      <td className="px-4 py-3 text-slate-600">{risco.status}</td>
                      <td className="px-4 py-3 text-slate-700">{risco.responsavel}</td>
                      <td className="px-4 py-3 text-slate-600">{risco.prazo_tratamento || "-"}</td>
                    </tr>
                  );
                })}
                ...
                {riscosFiltrados.length === 0 && (
                  <tr>
                    <td colSpan={16} className="px-6 py-16 text-center text-slate-400 font-bold">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-lg font-black text-slate-900">Mapa de Calor 5x5</h2>
              <p className="text-xs text-slate-500 font-medium">Probabilidade x Impacto</p>
            </div>
          ...
          <div className="grid grid-cols-[36px_repeat(5,1fr)] gap-2">
            ...
            {[5, 4, 3, 2, 1].map((p) => (
              <React.Fragment key={p}>
                <div className="flex items-center justify-center text-[10px] font-black text-slate-400">P{p}</div>
                {[1, 2, 3, 4, 5].map((i) => {
                  const score = p * i;
                  const qtd = riscos.filter((r) => r.probabilidade === p && r.impacto === i).length;

                  return (
                    <div
                      key={`${p}-${i}`}
                      className={`h-16 rounded-2xl border flex flex-col items-center justify-center ${classeCelula(score)}`}
                    >
                      <span className="text-sm font-black">{score}</span>
                      <span className="text-[10px] font-bold">{qtd} risco(s)</span>
                    </div>
                  );
                })}
              </React.Fragment>
          ...
          <div className="mt-6 space-y-3">
            {["BAIXO", "MODERADO", "ALTO", "CRITICO"].map((nivel) => (
              <div key={nivel} className="flex items-center justify-between text-xs">
                <span className={`px-2.5 py-1 rounded-lg border font-black ${classeRisco(nivel)}`}>{nivel}</span>
          ...
          <div className="mt-6 p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Avaliação baseada em probabilidade e impacto, com tratamento conforme prioridade,
              barreiras e controles para reduzir o risco residual.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-6xl max-h-[92vh] overflow-hidden rounded-3xl shadow-2xl border border-slate-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setModalAberto(false)} className="p-2 rounded-xl hover:bg-slate-100">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(92vh-160px)]">
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
                <Campo label="Código">
                  <input value={riscoEditando.codigo} onChange={(e) => atualizarCampo("codigo", e.target.value)} className="input" />
                </Campo>

                <Campo label="Categoria">
                  <select value={riscoEditando.categoria} onChange={(e) => atualizarCampo("categoria", e.target.value)} className="input">
                ...
                <Campo label="Setor">
                  <input value={riscoEditando.setor} onChange={(e) => atualizarCampo("setor", e.target.value)} className="input" />
                </Campo>

                <Campo label="Responsável">
                  <input value={riscoEditando.responsavel} onChange={(e) => atualizarCampo("responsavel", e.target.value)} className="input" />
                </Campo>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <Campo label="Processo vinculado">
                  <input value={riscoEditando.processo_vinculado || ""} onChange={(e) => atualizarCampo("processo_vinculado", e.target.value)} className="input" />
                </Campo>

                <Campo label="Origem do risco">
                  <select value={riscoEditando.origem || ""} onChange={(e) => atualizarCampo("origem", e.target.value)} className="input">
              ...
              <div className="grid grid-cols-1 gap-4 mb-6">
                <Campo label="Título do risco">
                  <input value={riscoEditando.titulo} onChange={(e) => atualizarCampo("titulo", e.target.value)} className="input" />
                </Campo>

                <Campo label="Descrição do risco">
                  <textarea value={riscoEditando.descricao || ""} onChange={(e) => atualizarCampo("descricao", e.target.value)} className="textarea" />
                </Campo>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <Campo label="Causa / fonte do risco">
                  <textarea value={riscoEditando.causa || ""} onChange={(e) => atualizarCampo("causa", e.target.value)} className="textarea" />
                </Campo>

                <Campo label="Consequência / impacto esperado">
                  <textarea value={riscoEditando.consequencia || ""} onChange={(e) => atualizarCampo("consequencia", e.target.value)} className="textarea" />
                </Campo>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
                <Campo label="Probabilidade">
                  <select value={riscoEditando.probabilidade} onChange={(e) => atualizarCampo("probabilidade", Number(e.target.value))} className="input">
                    {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                </Campo>

                <Campo label="Impacto">
                  <select value={riscoEditando.impacto} onChange={(e) => atualizarCampo("impacto", Number(e.target.value))} className="input">
                    {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                </Campo>

                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[10px] uppercase tracking-widest font-black text-slate-400 mb-2">Score inerente</p>
                  <div className={`inline-flex px-3 py-1.5 rounded-xl border text-sm font-black ${classeCelula(scoreAtual)}`}>
                    {scoreAtual} — {classificar(scoreAtual)}
                  </div>
                </div>

                <Campo label="Status">
                  <select value={riscoEditando.status} onChange={(e) => atualizarCampo("status", e.target.value)} className="input">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <Campo label="Controles existentes">
                  <textarea value={riscoEditando.controles_existentes || ""} onChange={(e) => atualizarCampo("controles_existentes", e.target.value)} className="textarea" />
                </Campo>

                <Campo label="Práticas de controle">
                  <textarea value={riscoEditando.praticas_controle || ""} onChange={(e) => atualizarCampo("praticas_controle", e.target.value)} className="textarea" />
                </Campo>

                <Campo label="Monitoramento / evidência">
                  <textarea value={riscoEditando.monitoramento || ""} onChange={(e) => atualizarCampo("monitoramento", e.target.value)} className="textarea" />
                </Campo>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <Campo label="Barreiras preventivas">
                  <textarea value={riscoEditando.barreiras_preventivas || ""} onChange={(e) => atualizarCampo("barreiras_preventivas", e.target.value)} className="textarea" />
                </Campo>

                <Campo label="Barreiras detectivas">
                  <textarea value={riscoEditando.barreiras_detectivas || ""} onChange={(e) => atualizarCampo("barreiras_detectivas", e.target.value)} className="textarea" />
                </Campo>

                <Campo label="Barreiras corretivas">
                  <textarea value={riscoEditando.barreiras_corretivas || ""} onChange={(e) => atualizarCampo("barreiras_corretivas", e.target.value)} className="textarea" />
                </Campo>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
                <Campo label="Estratégia de tratamento">
                  <select value={riscoEditando.estrategia} onChange={(e) => atualizarCampo("estrategia", e.target.value)} className="input">
                ...
                <Campo label="Prazo de tratamento">
                  <input type="date" value={riscoEditando.prazo_tratamento || ""} onChange={(e) => atualizarCampo("prazo_tratamento", e.target.value)} className="input" />
                </Campo>

                <Campo label="Data de avaliação">
                  <input type="date" value={riscoEditando.data_avaliacao || ""} onChange={(e) => atualizarCampo("data_avaliacao", e.target.value)} className="input" />
                </Campo>

                <Campo label="Próxima revisão">
                  <input type="date" value={riscoEditando.data_revisao || ""} onChange={(e) => atualizarCampo("data_revisao", e.target.value)} className="input" />
                </Campo>
              </div>

              <Campo label="Plano de tratamento">
                <textarea value={riscoEditando.plano_tratamento || ""} onChange={(e) => atualizarCampo("plano_tratamento", e.target.value)} className="textarea min-h-[110px]" />
              </Campo>

              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mt-6">
                <Campo label="Probabilidade residual">
                  <select value={riscoEditando.risco_residual_probabilidade || 1} onChange={(e) => atualizarCampo("risco_residual_probabilidade", Number(e.target.value))} className="input">
                    {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                </Campo>

                <Campo label="Impacto residual">
                  <select value={riscoEditando.risco_residual_impacto || 1} onChange={(e) => atualizarCampo("risco_residual_impacto", Number(e.target.value))} className="input">
                    {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                </Campo>

                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[10px] uppercase tracking-widest font-black text-slate-400 mb-2">Score residual</p>
                  <div className={`inline-flex px-3 py-1.5 rounded-xl border text-sm font-black ${classeCelula(scoreResidual)}`}>
                    {scoreResidual} — {classificar(scoreResidual)}
                  </div>
                </div>

                <Campo label="KPI relacionado">
                  <input value={riscoEditando.kpi_relacionado || ""} onChange={(e) => atualizarCampo("kpi_relacionado", e.target.value)} className="input" />
                </Campo>
              </div>
            </div>
            ...
            <div className="p-5 border-t border-slate-100 flex items-center justify-between bg-slate-50">
              ...
              <div className="flex gap-3">
                <button onClick={() => setModalAberto(false)} className="px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 text-sm font-bold">
                  Cancelar
                </button>
