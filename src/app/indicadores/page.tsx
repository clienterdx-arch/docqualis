...
import {
  ...
  Search, ChevronDown, FileCheck, Activity, Plus, FileText,
  MessageSquarePlus, Eye, PieChart, BarChart3
} from "lucide-react";
...
const ALL_INDICATORS = (() => {
  const all: {
    id: string; nome: string; setor: string; setorSigla: string; cor: string;
    tipo: string; frequencia: string; polaridade: string; unidade: string;
    status: string; meta: number; valorAtual: number; historico: number[];
    delta: string; tendencia: string; responsavel: string; ultimaAnalise: string;
    semAnalise: boolean; pendenteAlimentacao: boolean;
    consolidadoAnterior: number; consolidadoAtual: number; metaConsolidada: number;
  }[] = [];
  let c = 1;
  SETORES.forEach((setor, si) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const history = Array.from({ length: 12 }).map((_, idx) =>
        parseFloat((meta + (sr(seed + idx) - 0.5) * 30).toFixed(1))
      );
      all.push({
        ...
        tipo: "Resultado", frequencia: "Mensal",
        polaridade: r(6) > 0.5 ? "Maior Melhor" : "Menor Melhor",
        unidade: i % 2 === 0 ? "%" : "un",
        status: r(1) < 0.55 ? "verde" : r(1) < 0.78 ? "amarelo" : "vermelho",
...
const BORDER_LIGHT_BLUE = "#d9d9d9";
...
function TIcon({ val, target, pol, t }: { val?: number; target?: number; pol?: string; t?: string }) {
...
function PremiumChart({ data, config, faixas }: { data: Record<string, number | null | string>[]; config: ChartConfig; metaStyle: MetaStyle; faixas: Faixa[] }) {
  const W = 900, H = 420;
  const PAD = { t: 60, r: 30, b: 40, l: 45 };
  const cW = W - PAD.l - PAD.r;
  const cH = H - PAD.t - PAD.b;

  const validData = data.filter((d) => d.realizado != null);
  const globalMeta = data.length > 0 ? (data[0].meta as number) : 27;

  const activeExtras = (config.extraSeries || []).filter((s) => s.active);
  const allSeries = [
  ...
  let calcMin = 0, calcMax = 100;
  if (validData.length > 0) {
    const allValues = validData.flatMap((d) => allSeries.map((s) => Number(d[s.key]) || 0));
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} className="block overflow-visible font-sans select-none">
      ...
      {config.mostrarFaixas && faixas.map((f, i) => {
      ...
      {barSeries.map((series, sIdx) =>
        validData.map((d, i) => {
          ...
          return <rect key={`bar-${series.key}-${i}`} x={xPos} y={yPos} width={singleBarW} height={height} fill={series.color} className="transition-all duration-300 opacity-95 hover:opacity-100" />;
        })
      )}
      {lineSeries.map((series) => {
        const pts = validData.filter((d) => d[series.key] != null).map((d, i) => `${toX(i)},${toY(Number(d[series.key]))}`).join(" L ");
            ...
            <g key={`lbl-${series.key}-${i}`} className="transition-all duration-500">
              ...
              {config.mostrarRotulos && (
                <text x={cx} y={cy - 12} textAnchor="middle" fill={LABEL_COLOR} fontSize="13" fontWeight="500">
                  {formatValue(Number(val), config.formato, config.decimais)}
                </text>
              )}
            </g>
          );
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={`x-${i}`}>
            ...
            <text x={cx} y={PAD.t + cH + 22} textAnchor="middle" fill="#000" fontSize="12" fontWeight="600">{d.periodo as string}</text>
          </g>
        );
      })}
    </svg>
...
// ─── MINI BAR CHART ───────────────────────────────────────────────────────────
function MiniBarChart({ title, labelA1, labelA2, valA, colorA, labelB1, labelB2, valB, colorB, showTrendLine, tipo }: {
  title: string; labelA1: string; labelA2: string; valA: number; colorA: string;
  labelB1: string; labelB2: string; valB: number; colorB: string; showTrendLine?: boolean; tipo: string;
}) {
  const W = 200, H = 360;
...
// ─── COMPARISON CHART ─────────────────────────────────────────────────────────
function IndicatorComparisonChart({ ind1Data, ind2Data, config1, config2 }: {
  ind1Data: Record<string, number | null | string>[];
  ind2Data: Record<string, number | null | string>[];
  config1: { formato: string };
  config2: { formato: string };
}) {
  const W = 1000, H = 400;
  const PAD = { t: 60, r: 60, b: 60, l: 60 };
  const cW = W - PAD.l - PAD.r, cH = H - PAD.t - PAD.b;
  const valid1 = ind1Data.filter((d) => d.realizado != null);
  const valid2 = ind2Data.filter((d) => d.realizado != null);
  const vMax1 = 100, vMax2 = 120;
  ...
  const pts1 = valid1.map((d, i) => `${toX(i, valid1.length)},${toY1(Number(d.realizado))}`).join(" L ");
  const pts2 = valid2.map((d, i) => `${toX(i, valid2.length)},${toY2(Number(d.realizado))}`).join(" L ");
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} className="block overflow-visible font-sans select-none">
      ...
      {valid1.map((d, i) => <circle key={`p1-${i}`} cx={toX(i, valid1.length)} cy={toY1(Number(d.realizado))} r="5" fill="#fff" stroke={MAIN_COLOR} strokeWidth="2.5" />)}
      {valid2.map((d, i) => <circle key={`p2-${i}`} cx={toX(i, valid2.length)} cy={toY2(Number(d.realizado))} r="5" fill="#fff" stroke="#818cf8" strokeWidth="2.5" />)}
      {ind1Data.map((d, i) => <text key={`x-${i}`} x={toX(i, ind1Data.length)} y={H - 20} textAnchor="middle" fill="#94a3b8" fontSize="11" fontWeight="600">{d.periodo as string}</text>)}
    </svg>
  );
}
...
function IndicadorDetailView({ ind, onBack }: { ind: Indicator; onBack: () => void }) {
  ...
  const [dataSeries1, setDataSeries1] = useState<DataRow[]>(
    INITIAL_DATA_IND1.map((d) => ({ ...d, extra1: null, extra2: null, extra3: null, extra4: null }))
  );
  ...
  const [kpiConfig1, setKpiConfig1] = useState({
    nome: ind.nome, setor: ind.setor,
    formato: ind.unidade === "%" ? "percentual" : "absoluto",
    decimais: 1, polaridade: ind.polaridade, agrupamento: 1,
  });

  const [chartConfig1, setChartConfig1] = useState<Omit<ChartConfig, "formato" | "decimais" | "extraSeries">>({
    tipoGrafico: "bar", eixoYAuto: true, eixoYMin: 0, eixoYMax: 100,
    mostrarRotulos: true, mostrarFaixas: false, mostrarEixoY: true, mostrarMeta: true,
  });

  const [metaStyle, setMetaStyle] = useState<MetaStyle>({
    mostrar: true, valor: ind.meta, cor: TARGET_RED, espessura: 1.5, tracejada: true, rotulo: "META",
  });
  ...
  const validData2 = INITIAL_DATA_IND2.filter((d) => d.realizado != null);
  const ytdAtual2 = validData2.length > 0 ? validData2.reduce((a, b) => a + b.realizado, 0) / validData2.length : 0;
  const metaAnual2 = 85.0;

  const aggregatedData1 = useMemo(() => {
    if (kpiConfig1.agrupamento === 1) return dataSeries1 as Record<string, number | null | string>[];
    const chunked: Record<string, number | null | string>[] = [];
    for (let i = 0; i < dataSeries1.length; i += kpiConfig1.agrupamento) {
      ...
      const aggRecord: Record<string, number | null | string> = {
        periodo: kpiConfig1.agrupamento === 3 ? `T${i / 3 + 1}` : kpiConfig1.agrupamento === 6 ? `S${i / 6 + 1}` : `P${i / kpiConfig1.agrupamento + 1}`,
        meta: slice[0].meta,
        realizado: valids.length > 0 ? valids.reduce((a, b) => a + (b.realizado ?? 0), 0) / valids.length : null,
      };
      extraSeries.filter((es) => es.active).forEach((es) => {
        const esValids = slice.filter((s) => s[es.key as keyof DataRow] != null);
        aggRecord[es.key] = esValids.length > 0 ? esValids.reduce((a, b) => a + ((b[es.key as keyof DataRow] as number) ?? 0), 0) / esValids.length : null;
      });
  ...
  const handleUpdateTableData = (idx: number, field: string, val: string) => {
    const newSeries = [...dataSeries1];
    (newSeries[idx] as Record<string, unknown>)[field] = val === "" ? null : field === "periodo" ? val : Number(val);
    setDataSeries1(newSeries);
  };

  const handleAddMonth = () => {
    const newId = dataSeries1.length > 0 ? Math.max(...dataSeries1.map((d) => d.id)) + 1 : 1;
    setDataSeries1([...dataSeries1, { id: newId, periodo: "Novo", realizado: null, meta: metaStyle.valor, extra1: null, extra2: null, extra3: null, extra4: null }]);
  };

  const handleDeleteMonth = (id: number) => setDataSeries1(dataSeries1.filter((d) => d.id !== id));

  const handleUpdateExtraSeries = (key: string, field: string, val: unknown) => {
    setExtraSeries((prev) => prev.map((s) => s.key === key ? { ...s, [field]: val } : s));
  };

  const fullChartConfig: ChartConfig = { ...chartConfig1, formato: kpiConfig1.formato, decimais: kpiConfig1.decimais, extraSeries };

  return (
    <div className="flex flex-col h-full gap-8 bg-[#f8fafc]">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shrink-0 bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
        <div className="flex items-center gap-5">
          <button onClick={onBack} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all shadow-sm">
        ...
        <div className="flex items-center gap-6 border-l border-slate-100 pl-6">
          <div>
            ...
            <span className="text-2xl font-black text-[#2655e8] leading-none tracking-tight">{formatValue(ultimoMes1.realizado as number, kpiConfig1.formato, kpiConfig1.decimais)}</span>
          </div>
          <div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm flex flex-col shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[500px]">
          <div className="lg:col-span-8 min-w-0 relative">
            ...
            <PremiumChart data={aggregatedData1} config={fullChartConfig} metaStyle={metaStyle} faixas={faixas} />
          </div>
          <div className="lg:col-span-2">
            <MiniBarChart title="Desempenho Atual x Meta Global" labelA1="Meta" labelA2="" valA={metaAnual1} colorA={TARGET_RED} labelB1="Acum" labelB2="2026" valB={ytdAtual1} colorB={MAIN_COLOR} tipo={kpiConfig1.formato} />
          </div>
          <div className="lg:col-span-2">
            <MiniBarChart title="Consolidado Ano Ant. vs Atual (YTD)" labelA1="Acum" labelA2="2025" valA={prevYtd1} colorA="#94a3b8" labelB1="Acum" labelB2="2026" valB={ytdAtual1} colorB={MAIN_COLOR} tipo={kpiConfig1.formato} showTrendLine />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] shadow-sm flex-1 flex flex-col overflow-hidden min-h-[600px]">
        <div className="flex border-b border-slate-100 px-8 pt-2 bg-slate-50/80 overflow-x-auto shrink-0 gap-8">
          {[
            ...
            <button key={t.id} onClick={() => { setActiveTab(t.id as typeof activeTab); if (t.id === "capa") setCapaView("list"); }}
              className={`py-5 flex items-center gap-2 text-sm font-bold border-b-[3px] whitespace-nowrap outline-none transition-colors ${activeTab === t.id ? "text-indigo-700 border-indigo-600" : "text-slate-400 border-transparent hover:text-slate-700 hover:border-slate-300"}`}>
                      ...
                      return (
                        <tr key={row.id} className="transition-colors group hover:bg-slate-50">
                          ...
                          <td className="px-6 py-2.5 border-r border-slate-100">
                            <input type="number" value={row.realizado === null ? "" : row.realizado} onChange={(e) => handleUpdateTableData(idx, "realizado", e.target.value)} className="w-full max-w-[140px] px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all bg-white shadow-sm" placeholder="-" />
                          </td>
                          {extraSeries.filter((s) => s.active).map((s) => (
                            <td key={`cell-${s.key}-${row.id}`} className="px-6 py-2.5 border-r border-slate-100">
                              <input type="number" value={(row as Record<string, unknown>)[s.key] == null ? "" : String((row as Record<string, unknown>)[s.key])} onChange={(e) => handleUpdateTableData(idx, s.key, e.target.value)} className="w-full max-w-[140px] px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all bg-white shadow-sm" placeholder="-" />
                            </td>
                          ))}
                          ...
                          <td className="px-6 py-2.5 text-center">
                            <button onClick={() => handleDeleteMonth(row.id)} className="p-2.5 rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all flex mx-auto" title="Apagar Mês"><Trash2 className="w-4 h-4" /></button>
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center mb-6 shrink-0">
                ...
                <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-2xl border border-slate-100 shadow-inner">
                  ...
                  <select className="bg-transparent text-xs font-bold px-3 py-2 outline-none"><option>{INDICATORS[0].name}</option></select>
                  <span className="text-xs text-slate-400 font-black">vs</span>
                  <select className="bg-transparent text-xs font-bold px-3 py-2 outline-none"><option>{INDICATORS[1].name}</option></select>
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 bg-white border border-slate-200 rounded-3xl shadow-sm p-6 flex flex-col h-[480px]">
                  ...
                  <div className="flex-1 w-full min-h-0 relative">
                    <IndicatorComparisonChart ind1Data={dataSeries1 as Record<string, number | null | string>[]} ind2Data={INITIAL_DATA_IND2 as Record<string, number | null | string>[]} config1={{ formato: kpiConfig1.formato }} config2={{ formato: "numero" }} />
                  </div>
                </div>
                <div className="lg:col-span-4 space-y-6">
                  <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 flex flex-col gap-4">
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="text-3xl font-black text-indigo-600">{formatValue(ytdAtual1, kpiConfig1.formato, kpiConfig1.decimais)}</span>
                      <VarianceBadge value={ytdAtual1 - metaAnual1} />
                    </div>
                  </div>
                  <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 flex flex-col gap-4">
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="text-3xl font-black text-[#4f46e5]">{formatValue(ytdAtual2, "numero", 1)} {INDICATORS[1].unit}</span>
                      <VarianceBadge value={ytdAtual2 - metaAnual2} />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="max-w-4xl space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="md:col-span-2 space-y-6 bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                  <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 border-b border-slate-200 pb-2">Séries de Dados Adicionais (Até 4 extras)</h4>
                ...
                <div className="space-y-6 bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                  ...
                  <div className="space-y-4">
                    <label className="flex items-center justify-between cursor-pointer">
                      <span className="text-sm font-bold text-slate-700">Eixo Y Automático</span>
                      <input type="checkbox" checked={chartConfig1.eixoYAuto} onChange={() => setChartConfig1({ ...chartConfig1, eixoYAuto: !chartConfig1.eixoYAuto })} className="w-4 h-4 accent-indigo-600" />
                    </label>
                  ...
                  <label className="flex items-center justify-between cursor-pointer hover:bg-slate-100 p-1.5 rounded">
                    <span className="text-sm font-medium text-slate-700">Rótulos de Dados nos Pontos</span>
                    <input type="checkbox" checked={chartConfig1.mostrarRotulos} onChange={() => setChartConfig1({ ...chartConfig1, mostrarRotulos: !chartConfig1.mostrarRotulos })} className="w-4 h-4 accent-indigo-600" />
                  </label>
                </div>
                <div className="space-y-6 bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
          ...
          {activeTab === "config_kpi" && (
            <div className="max-w-3xl space-y-6">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm grid grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Polaridade</label>
                  <select value={kpiConfig1.polaridade} onChange={(e) => setKpiConfig1({ ...kpiConfig1, polaridade: e.target.value })} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm outline-none focus:border-indigo-500 bg-white">
                    <option>Maior Melhor</option>
                    <option>Menor Melhor</option>
                  </select>
                </div>
                <div>
              ...
              <div className="flex justify-end pt-4">
                <button className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white font-bold rounded-xl text-sm hover:bg-indigo-700 transition-all shadow-md"><Save className="w-4 h-4" /> Salvar Estrutura</button>
              </div>
            </div>
          )}
...
function IndicadoresList({ onSelect }: { onSelect: (ind: Indicator) => void }) {
  const [search, setSearch] = useState("");
  const [filterSetor, setFilterSetor] = useState("Todos");

  const filtered = ALL_INDICATORS.filter((ind) => {
    const matchSearch = ind.nome.toLowerCase().includes(search.toLowerCase()) || ind.id.toLowerCase().includes(search.toLowerCase());
    const matchSetor = filterSetor === "Todos" || ind.setor === filterSetor;
    return matchSearch && matchSetor;
  });

  const statusColors: Record<string, string> = { verde: "bg-emerald-500", amarelo: "bg-amber-400", vermelho: "bg-red-500" };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium mt-1">{ALL_INDICATORS.length} indicadores cadastrados · {filtered.length} exibidos</p>
        </div>
        <div className="flex gap-3">
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Buscar indicador..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-400 w-64 bg-white shadow-sm" />
          </div>
          <select value={filterSetor} onChange={(e) => setFilterSetor(e.target.value)} className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-400 bg-white shadow-sm font-medium text-slate-600">
            <option>Todos</option>
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((ind) => (
              <tr key={ind.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => onSelect(ind)}>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: ind.cor }} />
                    <div>
                      <p className="font-bold text-slate-800 text-sm">{ind.nome}</p>
                      <p className="text-[10px] font-mono text-slate-400">{ind.id}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className="text-xs font-bold px-2 py-1 rounded-md" style={{ backgroundColor: `${ind.cor}18`, color: ind.cor }}>{ind.setor}</span>
                </td>
                <td className="px-6 py-4 text-center">
                  <div className="flex items-center justify-center gap-1.5">
                    <div className={`w-2.5 h-2.5 rounded-full ${statusColors[ind.status]}`} />
                    <span className="text-xs font-bold text-slate-600 capitalize">{ind.status}</span>
                  </div>
                </td>
                <td className="px-6 py-4 text-right font-bold text-slate-800">{ind.valorAtual}{ind.unidade}</td>
                <td className="px-6 py-4 text-right font-medium text-slate-500">{ind.meta}{ind.unidade}</td>
                <td className="px-6 py-4 text-right">
                  <span className={`text-xs font-bold ${ind.valorAtual >= ind.meta ? "text-emerald-600" : "text-red-600"}`}>{ind.delta}</span>
                </td>
                <td className="px-6 py-4 text-center">
                  <span className="text-xs font-medium text-slate-600">{ind.responsavel}</span>
                </td>
                <td className="px-6 py-4 text-center">
                  <button className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-500 shadow-sm flex items-center gap-1 mx-auto group-hover:text-indigo-600 group-hover:border-indigo-200 transition-all">
...
// ─── PAGE (DEFAULT EXPORT — OBRIGATÓRIO PELO NEXT.JS) ─────────────────────────
export default function IndicadoresPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-6 md:p-10">
      {selectedIndicator ? (
        <IndicadorDetailView ind={selectedIndicator} onBack={() => setSelectedIndicator(null)} />
      ) : (
        <IndicadoresList onSelect={(ind) => setSelectedIndicator(ind)} />
      )}
    </div>
  );
}