...
const mockPerspectivas = [
  { id: "fin", nome: "Perspectiva Financeira", cor: "bg-emerald-50 border-emerald-100 text-emerald-800", icon: <TrendingUp className="w-5 h-5 text-emerald-600" /> },
  { id: "cli", nome: "Perspectiva Clientes & Mercado", cor: "bg-blue-50 border-blue-100 text-blue-800", icon: <Users className="w-5 h-5 text-blue-600" /> },
  { id: "pro", nome: "Perspectiva Processos Internos", cor: "bg-purple-50 border-purple-100 text-purple-800", icon: <Zap className="w-5 h-5 text-purple-600" /> },
  { id: "apr", nome: "Perspectiva Aprendizado & Crescimento", cor: "bg-amber-50 border-amber-100 text-amber-800", icon: <Briefcase className="w-5 h-5 text-amber-600" /> },
];

const mockObjetivos: any[] = [];
const mockIndicadores: any[] = [];
const mockIniciativas: any[] = [];
...
export default function PlanejamentoEstrategicoPage() {
  ...
  const getStatusBadge = (status: string) => {
    if (status === "No Alvo" || status === "Concluído") return "text-emerald-700 bg-emerald-50 border-emerald-100";
    if (status === "Atenção" || status === "Em Andamento") return "text-amber-700 bg-amber-50 border-amber-100";
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto animate-in fade-in duration-500 relative">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
             <div className="flex items-center gap-6">
                <div className="text-center">
                  ...
                  <p className="text-3xl font-black text-slate-800">0<span className="text-lg text-slate-500">%</span></p>
                </div>
              ...
              return (
                <div key={perspectiva.id} className={`p-6 rounded-2xl border ${perspectiva.cor} flex flex-col lg:flex-row gap-6 relative overflow-hidden group`}>
                  ...
                  <div className="flex-1 flex flex-wrap gap-4 items-center">
                    {objsDaPerspectiva.length === 0 ? (
                      <div className="text-sm font-bold opacity-50">Nenhum objetivo cadastrado nesta perspectiva.</div>
                    ) : (
      ...
      {viewState === "indicadores" && (
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden animate-in slide-in-from-right-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-50">
                {mockIndicadores.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-10 text-center text-sm font-bold text-slate-400">
                      Nenhum indicador estratégico cadastrado.
                    </td>
                  </tr>
                ) : (
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden animate-in slide-in-from-right-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-50">
                {mockIniciativas.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-10 text-center text-sm font-bold text-slate-400">
